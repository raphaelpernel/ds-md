...
export type ProductCardProps = ProductCardCatalogProps | ProductCardCartProps

export function ProductCard(props: ProductCardProps) {
  const { product, role } = props
  const isCart = props.context === 'cart'
  const orientation = isCart ? 'horizontal' : (props.orientation ?? 'vertical')
  const isHorizontal = orientation === 'horizontal'

  const cart = useCartOptional()
  const [localActivated, setLocalActivated] = useState(false)

  const catalogProps = !isCart ? props : null
