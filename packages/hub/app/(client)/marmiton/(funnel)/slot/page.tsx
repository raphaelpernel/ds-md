...
import { MOCK_TIMESLOT_DAYS } from '@/features/marmiton-prototype/data/mock/timeslots'
import type { Timeslot } from '@/features/marmiton-prototype/data/types/timeslot'
...
export default function CreneauPage() {
  ...
  const handleConfirm = () => {
    if (selected) router.push('/marmiton/payment')
  }
  ...
  return (
    <div className="order-timeslot-page" data-partner="carrefour">
      ...
      <div className="order-timeslot-breadcrumb-wrap">
        <Breadcrumb
          items={[
            { label: 'Magasin', href: '/marmiton/store' },
            { label: 'Créneau' },
            { label: 'Paiement' },
          ]}
          separator="›"
      ...
      {state.storeName && (
        <div className="order-timeslot-store-row">
          ...
          <Button variant="tertiary" size="S" onClick={() => router.push('/marmiton/store')}>
            Modifier
          </Button>
        </div>
      )}
