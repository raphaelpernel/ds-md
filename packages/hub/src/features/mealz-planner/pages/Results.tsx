...
import { pickRecipes } from '@/features/mealz-planner/data/mock/recipes'
...
export default function PlannerResultsPage() {
  ...
  const { state, reset, basePath } = usePlanner()
  const recipes = pickRecipes(state)

  const handleRestart = () => {
    reset()
    router.push(plannerPath(basePath, 'home'))
  }
  ...
  return (
    <div className="planner-results-page">
      ...
      <style>{`
        .planner-feature .planner-results-page {
          min-height: 100vh; max-width: 480px; margin: 0 auto;
          background: var(--color-surface-page); font-family: var(--font-family-body);
          display: flex; flex-direction: column;
        }

        .planner-feature .planner-results-page__header {
          padding: var(--spacing-16);
          border-bottom: 1px solid var(--color-border-default);
        }
        .planner-feature .planner-results-page__title {
          font-family: var(--font-family-heading);
          font-size: var(--font-size-heading-lg);
          line-height: var(--line-height-heading-lg);
          color: var(--color-content-default);
        }
        .planner-feature .planner-results-page__subtitle {
          font-size: var(--font-size-body-md);
          color: var(--color-content-weak);
          margin-top: var(--spacing-4);
        }

        .planner-feature .planner-results-page__list {
          flex: 1; list-style: none; margin: 0;
          display: flex; flex-direction: column; gap: var(--spacing-12);
          padding: var(--spacing-16);
        }

        .planner-feature .wizard-result-card {
          display: flex; align-items: center; gap: var(--spacing-12);
          padding: var(--spacing-12);
          border: 1px solid var(--color-border-weak);
          border-radius: var(--shape-card);
        }
        .planner-feature .wizard-result-card__thumb {
          width: 56px; height: 56px; flex-shrink: 0;
          border-radius: var(--shape-square);
          background: var(--color-surface-secondary);
        }
        .planner-feature .wizard-result-card__body { display: flex; flex-direction: column; gap: var(--spacing-8); }
        .planner-feature .wizard-result-card__name {
          font-size: var(--font-size-body-md);
          color: var(--color-content-default);
          margin: 0;
        }
        .planner-feature .wizard-result-card__meta { display: flex; gap: var(--spacing-8); }

        .planner-feature .planner-results-page__footer {
          position: sticky; bottom: 0;
          background: var(--color-surface-page); border-top: 1px solid var(--color-border-default);
          padding: var(--spacing-16);
        }
        .planner-feature .planner-results-page__cta { width: 100%; }
      `}</style>
    </div>
  )
}
