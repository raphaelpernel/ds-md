...
import { formatPrice } from '@/features/assistant-shopping/lib/format'
    ...
    <WidgetCard>
      <div className="product-carousel" role="list">
        {products.map((product) => {
          ...
          return (
            <article key={product.id} className="assistant-shopping-product-card" role="listitem">
              <button
                type="button"
                className="assistant-shopping-product-card__media"
                onClick={() => openProductChoice(productIds, product.id)}
                aria-label={`Voir le détail de ${product.name}`}
              >
                <span className="assistant-shopping-product-card__emoji" aria-hidden="true">{product.emoji}</span>
              </button>

              <div className="assistant-shopping-product-card__body">
                <button type="button" className="assistant-shopping-product-card__name" onClick={() => openProductChoice(productIds, product.id)}>
                  {product.name}
                </button>
                <p className="assistant-shopping-product-card__brand">{product.brand}{product.unit ? ` · ${product.unit}` : ''}</p>
                <p className="assistant-shopping-product-card__price">{formatPrice(product.price)}</p>

                {added ? (
                  <span className="assistant-shopping-product-card__added">
