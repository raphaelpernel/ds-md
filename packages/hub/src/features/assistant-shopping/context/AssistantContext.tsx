...
import { interpretMessage } from '@/features/assistant-shopping/lib/nlu'
import { mergeRecipeIntoCart, resolveRecipeProducts } from '@/features/assistant-shopping/lib/recipeProducts'
import { addProductIdsToCart } from '@/features/assistant-shopping/lib/cart'
import { MOCK_PRODUCTS } from '@/features/assistant-shopping/data/mock/products'
import { MOCK_RECIPES } from '@/features/assistant-shopping/data/mock/recipes'
import type { ChatMessage, FullViewState, PendingAction } from '@/features/assistant-shopping/data/types/chat'
import type { Store } from '@/features/assistant-shopping/data/types/store'
import type { CartState } from '@/features/assistant-shopping/data/types/cart'
...
export function AssistantProvider({ children }: { children: ReactNode }) {
  ...
  const openStoreLocator = useCallback(() => {
    ...
    pushMessage({ role: 'assistant', text: 'Sélectionnez votre magasin :', widget: { type: 'store-locator', payload: { reason: 'manual' } } })
  }, [pushMessage])
  ...
  const requestAddRecipe = useCallback(
    ...
    (recipeId: string, guests?: number) => {
      if (!store) {
        ...
        pushMessage({
          role: 'assistant',
          text: 'Pour ajouter cette recette à votre panier, sélectionnez d\u2019abord votre magasin :',
  ...
  const requestAddProducts = useCallback(
    (requestId: string, productIds: string[]) => {
      ...
      if (!store) {
        ...
        pushMessage({
          role: 'assistant',
          text: 'Pour ajouter ces articles à votre panier, sélectionnez d\u2019abord votre magasin :',
          widget: { type: 'store-locator', payload: { reason: 'add-to-cart' } },
        })
        return
      }
